...
def extractFemaleNames(filename,letter):
  f=open(filename,'r+')
  found=re.findall('<td>(\d+)<\/td><td>\w+<\/td><td>('+letter+'\w+)<\/td>',f.read()) 
  l=[str(n[1])+" "+str(n[0]) for n in found]
  return sorted(l)
...
def extractMaleNames(filename,letter):
  f=open(filename,'r+')
  found=re.findall('<td>(\d+)<\/td><td>('+letter+'\w+)<\/td><td>\w+<\/td>',f.read()) 
  l=[str(n[1])+" "+str(n[0]) for n in found]
...
def extractDocAndPopularityDate(filename):
  f=open(filename,'r+')
  fstr=f.read()
  found2=re.findall('<h3 align="center">(\w+ \w+ \d+)<\/h3>',fstr)
  found1=re.findall('<h1>Popular Names by Birth Year<\/h1>(\w+ \d+, \d+)<\/td>',fstr) 
  return str(found1[0])+" and "+str(found2[0]) 
